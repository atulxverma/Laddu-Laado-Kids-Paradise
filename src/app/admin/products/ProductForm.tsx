...
import { ImagePlus, Trash2, X, Baby, Venus } from "lucide-react"
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4">
        <input
          required
          placeholder="Product Title (e.g. Velvet Party Dress)"
          className={inputStyle}
          onChange={(e) => setForm({ ...form, name: e.target.value })}
        />
        <textarea
          placeholder="Describe the product, material, fit..."
          rows={3}
      ...
      <div className="space-y-1.5">
        <label className="text-[10px] font-black uppercase text-gray-400">
          Available Sizes (Separate with comma)
        </label>
        <input
          required
          placeholder="e.g. 2-4Y, 4-6Y, S, M"
          className={inputStyle}
          onChange={(e) => setForm({ ...form, size: e.target.value })}
        />
        <p className="text-[9px] text-gray-400 italic">
          Jo size tum yahan likhoge, sirf wahi shop pe dikhenge.
        </p>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={addDetail}
            className="text-[10px] font-bold text-black border border-black px-2 py-1 rounded-md hover:bg-black hover:text-white transition-all"
          >
            + ADD FIELD
          </button>
        </div>
      ...
      <button
        disabled={loading}
        className="w-full bg-black text-white py-5 rounded-full text-xs font-black uppercase tracking-[0.2em] shadow-xl active:scale-95 transition-all disabled:opacity-50"
      >
